...
class CI_Products
{
    ...
    public function all_products($url = "", $sort = "", $page_index = "")
    {
        ...
        if (!empty($cat_data)) { //-------fetch data by category
            ...
            $t = 1;
        } else if (!empty($sub_data)) { //-------fetch data by subcategory
            $id = $sub_data->id;
            $t = 2;
            ...
            $sendArray = array("links" => [], "product_data" => [], "all_data" => [], "category_name" => [], "subcategory_name" => [], "url" => [], "id" => [], "t" => []);
            return $sendArray;
        }
        ...
        $all_data = $this->CI->db->get()->result();
        // print_r($product_data);die();
        $links = $this->CI->pagination->create_links();
        $sendArray = array("links" => $links, "product_data" => $product_data, "all_data" => $all_data, "category_name" => $category_name, "subcategory_name" => $subcategory_name, "url" => $url, "id" => $id, "t" => $t);
        return $sendArray;
    }
    //====================================== PRODUCT DETAIL ===============================================
    ...
    public function related_products($url,$category_id)
    {
        ...
        $related_products = $this->CI->db->limit(15)->get_where('tbl_product', array('url != ' => $url, 'is_active' => 1, 'category_id' => $category_id,  'cat_active' => 1, 'subcat_active' => 1, 'product_view != ' => $productviewnotequal, 'product_type != ' => 2));
    ...
    public function searchForProducts($string)
    {
        ...
        // print_r($product_data);die();
        // $links = $this->CI->pagination->create_links();
        $sendArray = array("links" => '', "product_data" => $product_data, "category_name" => $category_name, "subcategory_name" => $subcategory_name);
        return $sendArray;
    }
            ...
            foreach ($final_filter as $product) {
                ...
                foreach ($color as $colorfilter) {
                    ...
                    if ($product['colour_id'] == $colorfilter) {
                        // print_r($product);die();
                        $final_filter2[] = array(
                            'id' => $product['id'], 'product_view' => $product['product_view'], 'type_id' => $product['type_id'], 'spgst' => $product['spgst'], 'exclusive' => $product['exclusive'], 'url' => $product['url'], 'name' => $product['name'],
